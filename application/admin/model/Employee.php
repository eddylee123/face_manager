...
class   Employee extends Model
{
    ...
    {
        $str = "set nocount on;
        set nocount on;exec KwwSys.dbo.[GetNewEmpID] 'L33%s','00000','%s','ADMIN','GH03'
        set nocount off;";
    ...
    {
        $str = "set nocount on;
        exec KwwSys.dbo.[GetNewEmpID] 'Z33%s','00000','%s','ADMIN','GH01'
        set nocount off;";
        $sql = sprintf($str, $org_id, $org_id);
        $rs = Db::query($sql);
        $emp_id = '';
...
//                $msg = !empty($empErr[$arr['st']]) ? $empErr[$arr['st']] : '员工状态异常，请核实后操作';
    ...
    public function getUrgencyTelAttr($value, $data)
    {
        return !empty($value) ? trim($value) : $value;
    }
